...
const AccountSetting = () => {
  ...
  const [name, setName] = useInput(
    vendor.details?.name ? vendor.details?.name : '',
  );
  const [phone, setPhone] = useInput(
    vendor.details?.phone ? vendor.details?.phone : '',
  );
  const [email, setEmail] = useInput(
    vendor.details?.email ? vendor.details?.email : '',
  );
  ...
  const changePasswordHandler = async () => {
    ...
    const res = await changePassDataInput(data, vendor.auth);
    console.log(res);
    if (res?.code === 200) {
      return toast({
  ...
  return (
    <div>
      <Box mx={['5px', '10px', '50px', '150px']} mt="5" minH="400px">
        <Box boxShadow="md" bg="white" mb="3" rounded="md">
          <Box display="flex" p="3">
            <Text>Name :</Text>
            <Spacer />
            <Text>{vendor.details?.name}</Text>
            <Spacer />
            <Text>
              <Button
                colorScheme="teal"
